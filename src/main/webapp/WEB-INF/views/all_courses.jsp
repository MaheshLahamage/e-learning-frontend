<%@ include file="header.jsp" %>
<%@ include file="login_req.jsp" %>
<head>
<meta charset="ISO-8859-1">
<title>Course List</title>
<style>
	.btn{
	  width: 100px;
	}
	.button {
	  width: 100px; /* set a width so it doesnt change upon hover */
	}
	
	.button:hover span {
	  display:none
	}
	
	.button:hover:before {
	  content:"Unenroll";
	}
	
	.button:hover {
    color: #db4f4f;
    background-color: #fff;
    border: 0.5px solid #db4f4f;
    background-image: none !important;
    box-shadow: none;
    box-shadow: inset 0 0 2rem rgba(219, 79, 79, .08);
  }
</style>
</head>
<body>
	<div class="container jumbotron">
	<c:if test="${auth.roll == 'USER'}">
		<div class="panel-heading"><span class="lead">Enrolled Courses</span></div>
	            <div class="table">
			    	<div class="th">
                        <span class="td">Id</span>
                        <span class="td">Course Name</span>
                        <span class="td">Fee</span>
                        <span class="td">Descreption</span>
                        <span class="td">Resources</span>
                        <span class="td"></span>
                        <span class="td"></span>
                    </div>
	                <c:forEach items="${enrolled}" var="course">
	                    <div class="tr">
	                        <span class="td">${course.course_id}</span>
	                        <span class="td">${course.c_name}</span>
	                        <span class="td">${course.c_fees}</span>
	                        <span class="td">${course.c_desp}</span>
	                        <span class="td">${course.c_resource}</span>
	                        
		                        <form action="courseunenrollProcess" method="Post" id="form1${course.course_id}">
									<c:if test="${auth.roll == 'ADMIN'}">
										<input type="number" id="user_id" name="user_id">
									</c:if>
									<c:if test="${auth.roll == 'USER'}">
										<input type="hidden" id="user_id" name="user_id" value="${auth.obj.user_id}">
									</c:if>
									<input type="hidden" id="course_id" name="course_id" value="${course.course_id}">
								</form>
							
	                        <span class="td"><button type="submit" form="form1${course.course_id}" class="btn btn-outline-success button"><span>Enrolled</span></button></span>
	                    </div>
	                </c:forEach>
                </div>
             </c:if>
	<c:if test="${auth.roll == 'ADMIN'}">
	
		<div class="panel-heading"><span class="lead">Enrolled Courses</span></div>
	            <div class="table">
			    <div class="th">
	                        <span class="td">Course Id</span>
	                        <span class="td">Course Name</span>
	                        <span class="td">User Id</span>
	                        <span class="td">Username</span>
	                        <span class="td"></span>
	                        <span class="td"></span>
	               </div>
                <c:forEach items="${enrolledcourses}" var="course" varStatus="status">
	                    <div class="tr">
	                        <span class="td">${course.course_id}</span>
	                        <span class="td">${course.c_name}</span>
	                        <span class="td">${enrolledusers[status.index].user_id}</span>
	                        <span class="td">${enrolledusers[status.index].name}</span>
		                        <form action="courseunenrollProcess" method="Post" id="form1${course.course_id}">
									<c:if test="${auth.roll == 'ADMIN'}">
										<input type="hidden" id="user_id" name="user_id"  value="${enrolledusers[status.index].user_id}">
									</c:if>
									<c:if test="${auth.roll == 'USER'}">
										<input type="hidden" id="user_id" name="user_id" value="${auth.obj.user_id}">
									</c:if>
									<input type="hidden" id="course_id" name="course_id" value="${course.course_id}">
								</form>
							
	                        <span class="td"><button type="submit" form="form1${course.course_id}" class="btn btn-outline-success button"><span>Enrolled</span></button></span>
	                   </div>
	                </c:forEach>
	            </div>
	</c:if>

		<div class="panel-heading"><span class="lead">List of Courses </span></div>
	            <div class="table">
	                <div class="th">
	                        <span class="td">Id</span>
	                        <span class="td">Course Name</span>
	                        <span class="td">Fee</span>
	                        <span class="td">Descreption</span>
	                        <span class="td">Resources</span>
	                        <span class="td"></span>
	                        <span class="td"></span>
							<c:if test="${auth.roll == 'ADMIN'}">
		                        <span class="td"></span>
		                        <span class="td"></span>
	                        </c:if>
	                    </div>
	                <c:forEach items="${courses}" var="course">
	                    <div class="tr">
	                        <span class="td">${course.course_id}</span>
	                        <span class="td">${course.c_name}</span>
	                        <span class="td">${course.c_fees}</span>
	                        <span class="td">${course.c_desp}</span>
	                        <span class="td">${course.c_resource}</span>
		                        <form action="courseenrollProcess" method="Post" id="form${course.course_id}">
									<c:if test="${auth.roll == 'ADMIN'}">
										<span class="td"><input type="number" id="user_id" name="user_id"></span>
									</c:if>
									<c:if test="${auth.roll == 'USER'}">
										<input type="hidden" id="user_id" name="user_id" value="${auth.obj.user_id}">
									</c:if>
									<input type="hidden" id="course_id" name="course_id" value="${course.course_id}">
								</form>
	                        <span class="td"><button type="submit" form="form${course.course_id}" class="btn btn-outline-primary">Enroll</button></span>
	                        
						<c:if test="${auth.roll == 'ADMIN'}">
	                       	<form action="delCourseProcess" method="Post" id="delform${course.course_id}">
								<input type="hidden" id="course_id${course.course_id}" name="course_id" value="${course.course_id}">
	                        </form>
					        <span class="td">
		                        <button type="submit" form="delform${course.course_id}"class="btn btn-outline-danger">
		                        	Remove
		                        </button>
		                    </span>
	                    </c:if>
	                    </div>
					<c:set var="i" value="${course.course_id}" />
	                </c:forEach>
	                </div>
	            
	<c:if test="${auth.roll == 'ADMIN'}">
		<div class="panel-heading"><span class="lead">Add Course </span></div>
			<div class="table">
			    <form name="my-form" action="submit_course" method="Post" >
			    <div class="th">
                    <span class="td">Course Id</span>
                    <span class="td">Course Name</span>
                    <span class="td">Fee</span>
                    <span class="td">Description</span>
                    <span class="td">Resources</span>
                    <span class="td"></span>
                    <span class="td"></span>
                    <span class="td"></span>
                    <span class="td"></span>
			    </div>
			    <div class="tr">
			        <span class="td">${i+1}</span>
			        <span class="td"><input type="text" name="c_name"/></span>
			        <span class="td"><input type="number" name="c_fees"/></span>
			        <span class="td"><input type="text" name="c_desp"/></span>
			        <span class="td"><input type="text" name="c_resource"/></span>
		       
			        <span class="td">
                        <button type="submit" class="btn btn-outline-primary">
                        Add
                        </button>
                    </span>
                    <span class="td"></span>
                    <span class="td"></span>
                    <span class="td"></span>
                </div>
			    </form>
		    </div>
		    </c:if>
	   </div>
</body>

